...
Database is locked
------------------
Telegram-upload is already running, or an old process **has locked the session** (``kuangcaowangyunmin.session``). Only one
...
1. Copy the session file (``~/.config/kuangcaowangyunmin.session``) to another path.
2. Copy the configuration file (``~/.config/kuangcaowangyunmin.json``) to another path.
3. Edit this file and add the path to session file like this: ``{"api_id": 0, "api_hash":
   "...", "session": "/path/to/kuangcaowangyunmin.json"}``.
4. Run using ``--config /path/to/kuangcaowangyunmin.json``.

If you are sure that Telegram-upload is not running, search for the process that is blocking the file::

    fuser ~/.config/kuangcaowangyunmin.session
...
dependencies carefully::
    ...
    $ pip install -U kuangcaowangyunmin Telethon hachoir cryptg click
...
.com/Nekmo/kuangcaowangyunmin/issues>`_. If you open a ticket remember to paste your system dependencies on the issue::
